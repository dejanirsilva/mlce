...
# Python uses: 256 -> LayerNorm -> activation -> 128 -> LayerNorm -> activation -> 64 -> activation -> 1
function create_value_network()
    return Chain(
        Dense(2, 256),
        LayerNorm(256),
...
function create_policy_network()
    return Chain(
        Dense(2, 256),
        LayerNorm(256),
...
#########################################################
### Network Evaluation (matching Python exactly) ###
#########################################################

# Normalize state to [0,1] range (matching Python normalize_state)
function normalize_state(state::AbstractMatrix)
    return (state .- state_min) ./ (state_max .- state_min)
end

# Value function: just the network output (Python doesn't multiply by k^α)
function v_net(state::AbstractMatrix, θᵥ, v_core, stᵥ)
    state_norm = normalize_state(state)
    out, _ = v_core(state_norm, θᵥ, stᵥ)
    return out
end

# Policy function: network output + δ with small scaling (matching Python)
# Python: X = mlp(X, activation=relu); X = X * 1e-3 + δ
function π_net(state::AbstractMatrix, θπ, π_core, stπ)
    state_norm = normalize_state(state)
    out, _ = π_core(state_norm, θπ, stπ)
    return out .* 1e-3 .+ δ  # Initialize near δ, let network learn the rest
end
...
function compute_mdp(state::AbstractMatrix, i_vals::AbstractMatrix)
    K = state[1:1, :]      # Capital
    ...
    # Dividends with equity issuance cost (smooth approximation)
    # Python: E = (1 + λ * (E_star < 0)) * E_star
    indicator = 1.0 ./ (1.0 .+ exp.(30.0 .* E_star))  # ≈1 when E_star < 0
    E = (1.0 .+ λ .* indicator) .* E_star
    
    # State drift: μs = [K*(i-δ), -θ*logz]
    μK = K .* (i_vals .- δ)
    μz = -θ .* logz

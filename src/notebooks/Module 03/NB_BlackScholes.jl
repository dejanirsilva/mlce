...
# ╔═╡ fae38517-0677-45ce-ade7-0d4c96efd448
using Plots, LaTeXStrings, LinearAlgebra, Distributions, PGFPlotsX

# ╔═╡ cfa153d9-8879-48d8-a130-94004195eacb
md"""
## The Black-Scholes-Merton Model

The PDE for the Black-Scholes-Merton model can be written:
```math
  -r v_T(s) - \frac{v_T(s)}{\partial T} + \overline{r} \frac{\partial v_T(s)}{\partial s} + \sigma_S^2 \frac{\partial v_T(s)}{\partial s^2} = 0.
```
with terminal condition $v_0(s) = \max\{e^s - K,0\}$, where $\overline{r} \equiv r - \frac{1}{2}\sigma_S^2$ is the risk-adjusted drift.

Let's start by defining the struct for the Black-Scholes-Merton model.
"""

# ╔═╡ 7818c1a0-cf87-11f0-0dc0-95bf4dda60fa
...
# ╔═╡ b68b944d-4777-461c-a233-1aed2d9e83f2
# Instantiating the Black-Scholes-Merton model
m = BlackScholesModel()

# ╔═╡ a518d49e-bd50-48ef-b5c3-480045ccf797
md"""
This problem has a well-known closed-form solution.
"""

# ╔═╡ 05602578-9b42-4e9d-8778-0cf3fa337dbb
# Exact solution
...
# ╔═╡ 94f53210-065d-4e37-85ec-bda719cf1b5e
md"""
We can also solve this problem using a *finite-differences method*.

We fix a *forward difference* in time and consider two versions for the spatial derivative. 

Using a **forward difference** in the spatial derivative:
```math
  r\,v_i^n
  = -\frac{v_i^{n+1} - v_i^n}{\Delta t}
    + \overline{r}\color{#d55e00}{\,\frac{v_{i+1}^n - v_i^n}{\Delta s}}
    \color{#000000}{+ \frac{\sigma_S^2}{2}\,\frac{v_{i+1}^n - 2v_i^n + v_{i-1}^n}{\Delta s^2},}
```

Using a **backward difference** in the spatial derivative:
```math
r\,v_i^n
  = -\frac{v_i^{n+1} - v_i^n}{\Delta t}
...
p_u = \frac{\overline{r}\,\Delta t}{\Delta s}\,\mathbf{1}_{\text{F}}
       ...
       - \frac{\sigma_S^2\,\Delta t}{\Delta s^2}, 
       p_d = -\frac{\overline{r}\,\Delta t}{\Delta s}\,\mathbf{1}_{\text{B}}
       + \frac{\sigma_S^2\,\Delta t}{2\Delta s^2}.
```
"""

# ╔═╡ 0e188e1e-4124-4fac-920a-493ca9d2ee5e
md"""
...
$\mathbf{v}^0 = [v_0(s_1), v_0(s_2), \ldots, v_0(s_N)]^\top$,
and adjustment vector
$\mathbf{b} = [0, \ldots, 0,\, p_u\,e^{s_N}\,\Delta s]^\top$.
"""

# ╔═╡ 6acf5fcd-39ac-4bba-8f98-9d752e405763
# Implementing the explicit scheme
...
# ╔═╡ fa15dd0c-1c7d-4c20-abac-c54b2c28e086
begin
...
# ╔═╡ bddeb761-aa8b-4c3d-931d-15a04ca3cc1c
md"""
The backward difference is unstable when the drift is *positive*, while the positive difference is unstable when the drift is *negative*.

This motivates the use of a **upwind scheme**, where we use a forward difference when the drift is positive, and a backward difference when the drift is negative.

Even with an upwind scheme, the explicit method requires the time step $\Delta t$ to be sufficiently small, as required by the *CFL condition*. 

An alternative is to use an *implicit scheme*. 
"""

# ╔═╡ 30ee8c29-a471-4ed7-b190-084f93731708
md"""
The implicit version of the discretization of the Black--Scholes--Merton PDE is
```math
  r\,v_i^{n+1}
  = -\frac{v_i^{n+1} - v_i^n}{\Delta t}
...
This method has the advantage that it is stable for any value of $\Delta t$. 
"""

# ╔═╡ 7991eac6-79ce-4f82-98fd-9f835a29ce0a
# Implementing the implicit scheme
...
# ╔═╡ e6635089-181d-409a-8beb-71b4d2556e6c
begin
	m3  = BlackScholesModel(Nt = 25, Ns = 100, T = 1.0, r = 0.01, σS = 0.30)
	vi = fd_implicit(m3)
	bs = BlackScholes.([m3], m3.sgrid)
	
	p_vi = plot(m3.sgrid, bs, line = 3, label = "Black-Scholes", xlabel = L"s", ylabel = L"v_T(s)", 
	    legend = :topleft, foreground_color_legend=:transparent, background_color_legend = :transparent);
	    plot!(m3.sgrid, vi.v, line = 3, label = "Finite-difference (implicit)", linestyle = :dash)
end

# ╔═╡ 388eee38-3e59-44d7-aad6-7932d70dbb01
md"## Appendix"

# ╔═╡ 2afe6e75-d4a3-4891-8c01-f2a21720b43b
pgfplotsx()
...
manifest_format = "2.0"
project_hash = "e7011c2060e7d8dff612169e8e02f7be7f9b992a"

...
# ╔═╡ 0349198a-27f4-461c-869f-30ba38c726de
begin
	# Dependencies
	using Plots, LaTeXStrings, PlutoUI, PGFPlotsX, HypertextLiteral
	using ForwardDiff, Random, Distributions, LinearAlgebra
	using Lux , Optimisers, ForwardDiff, Zygote, StableRNGs
	pgfplotsx()
end;

# ╔═╡ b6c30a58-bbc7-11f0-16bd-633dcfde7bc1
md"""
# Two trees model
Consider an economy where a representative investor with log utility can invest in a riskless asset and _two_ risky assets, each one with dividends following the process:
```math
  \frac{d D_{i,t}}{D_{i,t}} = \mu dt + \sigma dB_{i,t}, \qquad i = 1,2,
```
Aggregate consumption is given by $C_t = D_{1,t} +D_{2,t}$, and the share of total consumption coming from the first tree is $s_t \equiv D_{1,t} /C_t$. Ito's lemma implies that:
```math
  d s_t = - 2 \sigma^2 s_t(1-s_t)(s_t -1/2) dt + \sigma s_t(1-s_t)(dB_{1,t}- dB_{2,t}). 
```
Here is the drift and diffusion of $s$. Notice that the drift and diffusion vanish when $s= 0$ or $s =1$, that is, they are _absorbing states_.
"""

# ╔═╡ f97f86ec-3fe0-446b-88f7-efef998624cb
begin
    # Volatility
	σ = 0.30
	
	# --- State variable range ---
    s_range_plt = range(0.0, 1.0, length=100)

    # --- Drift and diffusion functions ---
    μₛ(s) = @. -2 * σ^2 * s * (1 - s) * (s - 1/2)
    σₛ(s) = @. sqrt(2) * σ * s * (1 - s)

    μₛ_range = μₛ(s_range_plt)
    σₛ_range = σₛ(s_range_plt)

 	p_μ = plot(s_range_plt, μₛ_range, line = 3.0, label = "", xlab = L"s", 
			   title = L"\mathrm{Drift}\ \mu_s(s)", ylims = (-0.01, 0.01))
	p_σ = plot(s_range_plt, σₛ_range, line = 3.0, label = "", xlab = L"s", color = palette(:auto)[2], title = L"\mathrm{Diffusion}\ \sigma_s(s)", ylims = (0.0, 0.125))

	plot(p_μ, p_σ, size = (950, 380))
	
end

# ╔═╡ 56a0c453-377a-4c2d-b40f-b697c3735208
md"""
## The boundary-value problem
Let $P_t$ denote the price of the first tree (we can obtain the other price by symmetry). It is useful to work with the price-consumption ratio: $v_t \equiv P_t/C_t$. The pricing condition for a log investor implies:
```math
  v_{t} = \mathbb{E}_t \left[ \int_{0}^{\infty} e^{-\rho s} s_{t+s} ds \right].
```
As $s_t$ is a Markov process, we can write the price-consumption ratio as a function of $s_t$: $v_t = v(s_t)$. 
Because $s_t$ is Markov, $v_t = v(s_t)$ satisfies the stationary HJB equation:
```math
...
We are interested in solving this boundary-value problem. We start by defining the model struct. It includes not only the model parameters, but also functions to compute the drift and diffusion of $s$.
"""

# ╔═╡ 6e556e27-b26a-46b2-b0e8-1a6f66f07400
# Model parameters
@kwdef struct TwoTrees
	ρ::Float32 = 0.04f0
	σ::Float32 = sqrt(0.04f0)
	μ::Float32 = 0.02f0
	μₛ::Function = s -> @. -2f0 * σ^2 * s * (1-s) * (s-0.5f0)   # drift of s
	σₛ::Function = s -> @. sqrt(2f0) * σ * s * (1-s)            # diffusion of s
end;

# ╔═╡ 1716bf2e-e0e6-4fc6-991f-99204e52a509
md"""
Notice that the drift and diffusion are defined with built-in broadcast. Hence, we can seamlessly obtain the drift and diffusion for a matrix of $s$, which is useful when we need to evaluate these functions on a mini-batch. 
"""

# ╔═╡ 5efb157b-a974-432b-8a0f-d583ee7a5fa8
begin
	m = TwoTrees()
	vcat( m.μₛ( ones(Float32,1,6) ), m.σₛ( ones(Float32,1,6) ) )
end

# ╔═╡ 4621ae24-afca-4c58-96d2-47725eb39a7f
md"""
### Computing the drift
We define next the auxiliary function used to compute the drift using the hyper-dual approach.
"""

# ╔═╡ b603db5e-7b07-4aa1-8b6f-28bd726d2f26
# Hyper-dual approach to Ito's lemma
function drift_hyper(V::Function, s::AbstractMatrix, m::TwoTrees)
	F(ϵ) = V(s + m.σₛ(s)/sqrt(2f0)*ϵ + m.μₛ(s)/2f0*ϵ^2)
	return ForwardDiff.derivative(ϵ -> ForwardDiff.derivative(F, ϵ), 0.0f0)
end;

# ╔═╡ ca9034cc-98cb-4835-9f7a-9cecf08796c6
md"""
It is useful to run a small test to check whether we implemented the hyper-dual approach correctly. For a simple test function $V(s)$, we compute the drift exactly and also using the function above. The calculations agree at the machine-precision level, indicating this was properly implemented.
"""

# ╔═╡ 19dcc28f-b59f-48f0-a1ec-7b567dfd0e7f
begin
	...
	errors = maximum(abs.((drifts_exact - drifts_hyper)./drifts_exact))
end;

# ╔═╡ ebc22869-1428-4ad6-8824-fcd237132192
md"**Relative Error**: $(errors)."

# ╔═╡ 76aa08e0-92f4-4787-8057-3e53a2b78f27
md"""
### Defining the neural net
Next, we are going to represent the function $v(s)$ using a deep neural network with SiLU (also known as _swish_) activation. As in Chapter 4, we use the package `Lux.jl` for this.
"""

# ╔═╡ 80ab1238-9617-40ad-ad65-d38afa83a636
model = Chain(
	Dense(1 => 20, Lux.swish),
	Dense(20 => 20, Lux.swish),
	Dense(20 => 1)
)

# ╔═╡ 4e64c1e5-2939-486a-af6f-702a7208d13e
# Initialize the parameters
ps0, ls0 = Lux.setup(rng, model)

# ╔═╡ ffdc7080-ff62-43a2-8982-e80af51adf02
# Use Adam for optimization
opt = Adam(1e-3)

# ╔═╡ ce32e1d5-d408-483c-becf-47ca6d955331
# Initialize the optimizer state
os0 = Optimisers.setup(opt, ps0)

# ╔═╡ 23db16ea-3a8f-4e32-8ac7-5fdfd7a1bed7
md"""
## Defining the loss function
In this problem, there is no optimal control to solve for, so we only need to perform the policy evaluation step. We choose the first update rule, which avoids the complications of mixed-mode automatic differentiation.

The loss function is simply the mean-square deviation of the model prediction and a given target. 
"""

# ╔═╡ 78865a4c-4101-46d5-87fd-c0ad091e1d69
# Loss function
function loss_fn(ps, ls, s, target)
	return mean(abs2, model(s, ps, ls)[1] - target)
end

# ╔═╡ 7c70f162-8234-44f7-82eb-695f8f45ce6d
md"""
In the first formulation of the policy evaluation step, the target is given by:
```math
	target(s; \mathbf{\theta}^{j-1}) = v(s;\mathbf{\theta}^{j-1}) + \textrm{HJB}(s;\mathbf{\theta}^{j-1})\Delta t.
```
"""

# ╔═╡ 3eaa3ee2-73b9-4007-9bd0-4f4c0b26230d
# Target
function target(v, s, m; Δt = 0.2f0)
	v̅ = v(s)
	hjb = s + drift_hyper(v, s, m) - m.ρ * v̅
	return v̅ + hjb * Δt
end;

# ╔═╡ fdbaf68e-c4d0-4720-a129-30b3e5ff9551
md"""
## Training the NN
"""

# ╔═╡ 46b37041-58d1-4dab-8642-c8ab172fd765
begin
	# Initialize parameters, layer states, and opt. states
	ps, ls, os = ps0, ls0, os0	
	
	# # Training loop
	max_iter = 30_000
	loss_history = Float32[]
	for i = 1:max_iter
		s_batch = [zeros(Float32,8);rand(rng,Float32,128);ones(Float32,8);]'
		tgt 	= target(s-> model(s, ps, ls)[1], s_batch, m)
...
# ╔═╡ ebd2f38c-3e7e-445b-995b-4cd84ed88cfa
plot((1:max_iter)/1_000, loss_history, yaxis = :log10, xlab = "iteration (in thousands)", ylab = "loss", label = "", title = "Training history")

# ╔═╡ 7b10e57e-e844-4172-a4f4-0ed8e2ec226c
begin
	s_range = range(0f0, 1f0, 100)
	...
	plot(s_range, v_exact, label = "Exact", l = 3, alpha = 0.85,
		 legend = :topleft, foreground_color_legend=:transparent, background_color_legend = :transparent)
	plot!(s_range, v_pred, xlabel = L"s", ylabel = L"v(s)", 
		 label = "DNN", ls = :dash, l = 2)
end

# ╔═╡ 266e842f-d8bf-44df-b6c2-5b2b4f644cea
md"""
# Appendix
"""
...
manifest_format = "2.0"
project_hash = "53c0289ef12d756ba3b22fb9e02a5972b6c7bdeb"
...
git-tree-sha1 = "83e3ff2a6e37e91e39d06207de9a576607795f51"
...
git-tree-sha1 = "0dd06916d60d6cc7d1a7b2d3c895b5a261353cf9"
